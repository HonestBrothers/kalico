...
#
# The blend replaces a sharp corner with a tangent circular arc whose maximum
# deviation from the vertex is bounded by the EXTRUDED BEAD (delta = ratio *
# bead_width), not by a hand-tuned junction_deviation. These cover the pure
# geometry engine: the deviation bound, the arc's tangency and radius, the
# skip rules, and conservation of extruded filament across the spliced chain.
#
# Run: klippy-env/bin/python test/test_cornerblend.py
import math
import os
import sys
...
import corner_blend as cb  # noqa: E402
...
def test_plan_corner_90_geometry():
    ...
    worst = min(
        _point_to_seg((10, 0, 0), poly[i], poly[i + 1])
    ...
    print(
        "  plan_corner 90deg: r=%.4f t=%.4f dev=%.4f nchord=%d, arc on-radius,"
        " polyline within delta OK"
        % (P["r"], P["t"], P["dev"], len(P["pts"]) + 1)
    )
...
def test_plan_corner_skips():
    ...
    assert (
        cb.plan_corner(
            (0, 0, 0), (10, 0, 0), (20, 0.5, 0), 0.1, min_turn_deg=8.0
        )
        is None
    )
    ...
    assert (
        cb.plan_corner(
            (0, 0, 0), (10, 0, 0), (0.5, 0.2, 0), 0.1, max_turn_deg=150.0
        )
        is None
    )
...
def test_chord_length_reasonable():
    P = cb.plan_corner(
        (0, 0, 0), (10, 0, 0), (10, 10, 0), 0.2, blend_ratio=0.5, chord_len=0.4
    )
...
def test_blend_chain_conserves_extrusion():
    ...
    print(
        "  plan_blend_chain conserves E (%.4f), endpoints kept, corner_v=%.1f "
